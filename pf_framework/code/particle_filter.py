...
def sample_motion_model(odometry, particles):
    ...
    delta_rot1_hat = delta_rot1 + sample_normal(0, a1*abs(delta_rot1) + a2*abs(delta_trans))
    delta_rot2_hat = delta_rot2 + sample_normal(0, a1*abs(delta_rot2) + a2*abs(delta_trans))
    delta_trans_hat = delta_trans + sample_normal(0, a3*abs(delta_trans) + a4*(abs(delta_rot1) + abs(delta_rot2)))
    ...
    for particle in particles:
        ...
        new_particle['theta'] = particle['theta'] + delta_rot1_hat + delta_rot2_hat
...
def eval_sensor_model(sensor_data, particles, landmarks):
    ...
    for particle in particles:
        ...
        for i in range(len(ids)):
            lm_id = ids[i]
            m_range = ranges[i] #measured ranges
            lx = landmarks[lm_id][0]
            ly = landmarks[lm_id][1]
            # calculate expected range measurement
            m_range_exp = np.sqrt(np.square(lx-x)+np.square(ly-y))
            meas_likelihood = scipy.stats.norm.pdf(m_range, m_range_exp, sigma_r)
